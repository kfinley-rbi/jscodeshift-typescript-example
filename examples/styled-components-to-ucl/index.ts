...
export default function transformer(fileInfo: FileInfo, api: API) {
  ...
  root
    ...
    .forEach((nodePath) => {
      const { asObjectOrFunction } = processCss(j, nodePath.node.quasi);

      j(nodePath).replaceWith(asObjectOrFunction);
    });
...
const processFile = (
  ...
  if (activeElement?.component && addToImports)
...
function processCss(j: JSCodeshift, quasi: any) {
